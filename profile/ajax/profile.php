...
add_address();
		...
		function movie_submit(){
	  	GLOBAL $db;
		  if(isset($_GET['submit_movie']) && $_GET['submit_movie'] == 'true')
		  {
		   $name     = $_POST['name'];
		   $duration    = $_POST['duration'];
		   $rating    = $_POST['rating'];
		   $Query    = $db->prepare("INSERT INTO movies (movie_title, movie_duration, movie_rating) VALUES (?,?,?)");
		   $Query->execute([$name, $duration, $rating]);
		   if($Query){
		   $_SESSION['movie_success'] = "<i class='fa fa-check-circle'></i> Movie Successfully Added";
        	echo json_encode(array("error" => 'success'));
		   }
	...
	if(isset($_GET['action']) && $_GET['action'] == 'fetch'){
		...
		$Fetch_Query = $db->prepare("SELECT * FROM movies WHERE id = ?");
		$Fetch_Query->execute(array($movie_id));
		$Fetch_r = $Fetch_Query->fetchAll();
		foreach($Fetch_r as $row)
					{
						$output['id'] = $row['id'];
						$output['title'] = $row['movie_title'];
						$output['duration'] = $row['movie_duration'];
						$output['rating'] = $row['movie_rating'];
					}
	...
	function update_movie(){
		...
		if(isset($_GET['update_movie']) && $_GET['update_movie'] == 'true'){
			...
			$name_reg = "/^[a-zA-Z 0-9 ]+$/";
			if(preg_match($name_reg, $name)){
	      $Query = $db->prepare("UPDATE movies SET movie_title = ?,movie_duration=?,movie_rating=? WHERE id = ?");
	       $Query->execute(array($name,$duration,$rating, $movieId));
	      // print_r($Query);
	       if($Query){
	       	$_SESSION['movie_update'] = "<i class='fa fa-check-circle'></i> Movie successfully updated";
	       	echo json_encode(array('error' => 'success'));
	       }
			} else {

				echo json_encode(array('error' => 'pattren', 'msg' => 'Movie Title will be only character'));
			}
		}
	}
	update_movie();
	...
	function delete_movie(){
			if(isset($_GET['delete']) && $_GET['delete'] == 'true'){
			{
				...
				{	GLOBAL $db;
					$query = "DELETE FROM movies WHERE id = '".$_POST["MovieId"]."'";
					$statement = $db->prepare($query);
					$statement->execute();
					if($statement){
        			$_SESSION['movie_success'] = "<i class='fa fa-check-circle'></i> Movie Deleted";
        			echo json_encode(array("error" => 'success'));
        			}
		...
		function validate_movie(){
			if(isset($_GET['check']) && $_GET['check'] == 'true'){
			{
					...
					GLOBAL $db;
					$query = "SELECT  * FROM movies WHERE movie_title = '".$_POST["name"]."'";
